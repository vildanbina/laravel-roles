@if (session('message'))
    <div class="alert alert-{{ Session::get('status') }} status-box alert-dismissable fade show" role="alert">
        <a href="#" class="close" data-dismiss="alert" aria-label="close">
            &times;
            <span class="sr-only">
                {!! trans('laravelroles.flash-messages.close') !!}
            </span>
        </a>
        {!! session('message') !!}
    </div>
...
@if (session('success'))
    <div class="alert alert-success alert-dismissable fade show" role="alert">
        ...
        <h4>
            ...
            {!! trans('laravelroles.flash-messages.success') !!}
        </h4>
        {!! session('success') !!}
    </div>
@endif
...
@if (session('error'))
    <div class="alert alert-danger alert-dismissable fade show" role="alert">
        ...
        <h4>
            ...
            {!! trans('laravelroles.flash-messages.error') !!}
        </h4>
        {!! session('error') !!}
    </div>
@endif
...
@if (session('errors') && count($errors) > 0)
    <div class="alert alert-danger alert-dismissable fade show" role="alert">
        ...
        <h4>
            <i class="icon fa fas fa-warning fa-fw" aria-hidden="true"></i>
            <strong>
                {!! trans('laravelroles.flash-messages.whoops') !!}
            </strong>
            {!! trans('laravelroles.flash-messages.someProblems') !!}
        </h4>
        <ul>
