...
<div class="table-responsive permissions-table">
    <table class="table table-sm table-striped data-table permissions-table">
        <caption class="p-1 pb-0">
            @if($tabletype == 'normal')
                {!! trans_choice('laravelroles.permissions-table.caption', $items->count(), ['count' => $items->count()]) !!}
            @endif
            @if($tabletype == 'deleted')
                {!! trans_choice('laravelroles.permissions-deleted-table.caption', $items->count(), ['count' => $items->count()]) !!}
            @endif
        </caption>
        <thead class="thead">
        <tr>
            <th scope="col">
                {!! trans('laravelroles.permissions-table.id') !!}
            </th>
            <th scope="col">
                {!! trans('laravelroles.permissions-table.name') !!}
            </th>
            <th scope="col">
                {!! trans('laravelroles.permissions-table.slug') !!}
            </th>
            <th scope="col" class="hidden-xs">
                {!! trans('laravelroles.permissions-table.desc') !!}
            </th>
            <th scope="col" class="hidden-xs hidden-sm">
                {!! trans('laravelroles.permissions-table.roles') !!}
            </th>
            <th scope="col" class="hidden-xs hidden-sm ">
                {!! trans('laravelroles.permissions-table.createdAt') !!}
            </th>
            <th scope="col" class="hidden-xs hidden-sm ">
                {!! trans('laravelroles.permissions-table.updatedAt') !!}
            </th>
            @if($tabletype == 'deleted')
                <th scope="col" class="hidden-xs hidden-sm ">
                    {!! trans('laravelroles.permissions-table.deletedAt') !!}
                </th>
            @endif
            <th class="no-search no-sort " colspan="3">
                {!! trans('laravelroles.permissions-table.actions') !!}
            </th>
        </tr>
        </thead>
            ...
            @foreach($items as $item)
                <tr>
                    ...
                    @if($tabletype == 'normal')
                        ...
                        @endpermission
                        @permission(config('permission.permissions.delete'))
                        <td>
                    ...
                    @if($tabletype == 'deleted')
                        ...
                        <td>
                            <a class="btn btn-sm btn-outline-info btn-block" href="{{ route('laravelroles::permission-show-deleted', $item->id) }}" data-toggle="tooltip" title="{{ trans('laravelroles.tooltips.show-deleted-permission') }}">
                                {!! trans("laravelroles.buttons.show-deleted-permission") !!}
        ...
        @else
            <tr>
                <td>{!! trans("laravelroles.permissions-table.none") !!}</td>
                <td></td>
                <td></td>
