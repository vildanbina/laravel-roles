@php
    ...
    if($type == 'role') {
        ...
        $btnTooltip     = trans('laravelroles.tooltips.restore-role');
        $btnText        = trans("laravelroles.buttons.restore-role");
        $modalTitle     = trans('laravelroles.modals.restore_modal_title', ['type' => $type, 'item' => $item->name]);
        $modalMessage   = trans('laravelroles.modals.restore_modal_message', ['type' => $type, 'item' => $item->name]);
        $dataTarget     = '#confirmRestoreRoles';
    }
    if($type == 'permission') {
        $formAction     = route('laravelroles::permission-restore', $item->id);
        $btnTooltip     = trans('laravelroles.tooltips.restore-permission');
        $btnText        = trans("laravelroles.buttons.restore-permission");
        $modalTitle     = trans('laravelroles.modals.restore_modal_title', ['type' => $type, 'item' => $item->name]);
        $modalMessage   = trans('laravelroles.modals.restore_modal_message', ['type' => $type, 'item' => $item->name]);
    ...
    if($style == 'large' && $type == 'role') {
        $btnText    = trans("laravelroles.buttons.restore-role-large");
        $btnClass   .= ' btn-md mb-0';
        $formClass  = 'mb-0';
    }
    if($style == 'large' && $type == 'permission') {
        $btnText    = trans("laravelroles.buttons.restore-permission-large");
        $btnClass   .= ' btn-md mb-0';
        $formClass  = 'mb-0';
    }
