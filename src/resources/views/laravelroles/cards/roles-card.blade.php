    ...
    <div class="card">
        <div class="card-header bg-default">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span id="card_title">
                    {!! trans('laravelroles.titles.roles-card') !!}
                </span>
                ...
                @if(count($items) != 0)
                    @foreach($items as $itemKey => $item)
                        ...
                        <li id="accordion_roles_{{ $itemKey }}" class="list-group-item accordion @if($item['users']->count() > 0 || $item['permissions']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_roles_{{ $itemKey }}">
                            <div class="d-flex justify-content-between align-items-center" @if($item['users']->count() > 0 || $item['permissions']->count() > 0) data-toggle="tooltip" title="{{ trans("laravelroles.tooltips.show-hide") }}" @endif>
                                <span class="badge badge-default role-name">
                                    {!! trans('laravelroles.titles.role-card') !!} <strong class="{{ $indRoleTextClass }}">{{ $item['role']->name }}</strong>
                                </span>
                                <div>
                                    <span class="badge badge-light">
                                        <small>
                                            {{ trans('laravelroles.cards.level', ['level' => $item['role']->level]) }}
                                        </small>
                                    </span>
