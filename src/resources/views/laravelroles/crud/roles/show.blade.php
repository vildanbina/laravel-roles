...
@section(config('roles.titleExtended'))
    {!! trans('laravelroles.titles.show-role') !!}
@endsection
                ...
                <div class="{{ $containerClass }} {{ $bootstrapCardClasses }}">
                    <div class="{{ $containerHeaderClass }}">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span id="card_title">
                                @isset($typeDeleted)
                                    {!! trans('laravelroles.titles.show-role-deleted', ['name' => $item->name]) !!}
                                @else
                                    {!! trans('laravelroles.titles.show-role', ['name' => $item->name]) !!}
                                @endisset
                            </span>
                            <div class="pull-right">
                                @isset($typeDeleted)
                                    <a href="{{ route('laravelroles::roles-deleted') }}" class="btn btn-outline-danger btn-sm float-right" data-toggle="tooltip" data-placement="left" title="{{ trans('laravelroles.tooltips.back-roles-deleted') }}">
                                        <i class="fa fa-fw fa-reply-all" aria-hidden="true"></i>
                                        {!! trans('laravelroles.buttons.back-to-roles-deleted') !!}
                                    </a>
                                @else
                    ...
                    <div class="{{ $containerBodyClass }}">
                        <ul class="list-group">
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.role-id') !!}
                                <span class="badge badge-pill">
                                    {{ $item->id }}
                                </span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.role-name') !!}
                                <span class="badge badge-pill">
                                    {{ $item->name }}
                                </span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.role-desc') !!}
                                <span class="badge badge-pill">
                                    @if($item->desc)
                                        {{ $item->desc }}
                                    @else
                                        <span class="text-muted">
                                            {!! trans('laravelroles.cards.role-info-card.none') !!}
                                        </span>
                                    @endif
                                </span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.role-level') !!}
                                <span class="badge badge-pill">
                                    @if($item->level)
                                        {{ $item->level }}
                                    @else
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.role-permissions') !!}
                            ...
                            <li id="accordion_roles_users" class="list-group-item accordion @if($item['users']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_roles_users">
                                <div class="d-flex justify-content-between align-items-center" @if($item['users']->count() > 0) data-toggle="tooltip" title="{{ trans("laravelroles.tooltips.show-hide") }}" @endif>
                                    {!! trans('laravelroles.cards.role-info-card.role-users') !!}
                                    <span class="badge badge-pill badge-dark">
                                        @if($item['users']->count() > 0)
                                            {!! trans_choice('laravelroles.cards.users-count', count($item['users']), ['count' => count($item['users'])]) !!}
                                        @else
                                            {!! trans('laravelroles.cards.none-count') !!}
                                        @endif
                                    </span>
                                ...
                                @if($item['users']->count() > 0)
                                    <div id="collapse_roles_users" class="collapse" data-parent="#accordion_roles_users" >
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                {!! trans('laravelroles.cards.role-card.table-users-caption', ['role' => $item->name]) !!}
                                            </caption>
                                            <thead>
                                                <tr>
                            ...
                            <li id="accordion_roles_permissions" class="list-group-item accordion @if($item['permissions']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_roles_permissions">
                                <div class="d-flex justify-content-between align-items-center" @if($item['permissions']->count() > 0) data-toggle="tooltip" title="{{ trans("laravelroles.tooltips.show-hide") }}" @endif>
                                    {!! trans('laravelroles.cards.role-info-card.role-permissions') !!}
                                    <span class="badge badge-pill badge-dark">
                                        @if($item['permissions']->count() > 0)
                                            {!! trans_choice('laravelroles.cards.permissions-count', count($item['permissions']), ['count' => count($item['permissions'])]) !!}
                                        @else
                                            {!! trans('laravelroles.cards.none-count') !!}
                                        @endif
                                    </span>
                                ...
                                @if($item['permissions']->count() > 0)
                                    <div id="collapse_roles_permissions" class="collapse" data-parent="#accordion_roles_permissions" >
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                {!! trans('laravelroles.cards.role-card.table-permissions-caption', ['role' => $item->name]) !!}
                                            </caption>
                                            <thead>
                                                <tr>
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.created') !!}
                                <span class="badge badge-pill">
                                    {!! $item->created_at->format(trans('laravelroles.date-format')) !!}
                                </span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {!! trans('laravelroles.cards.role-info-card.updated') !!}
                                <span class="badge badge-pill">
                            ...
                            @if ($item->deleted_at)
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    {!! trans('laravelroles.cards.role-info-card.deleted') !!}
                                    <span class="badge badge-pill">
                        ...
                        <div class="row">
                            <div class="col-sm-6 mt-3">
                                ...
                                @else
                                    <a class="btn btn-sm btn-secondary btn-block text-white mb-0" href="{{ route('laravelroles::roles.edit', $item->id) }}" data-toggle="tooltip" title="{{ trans("laravelroles.tooltips.edit-role") }}">
