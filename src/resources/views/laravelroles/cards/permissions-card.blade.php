    ...
    <div class="card">
        <div class="card-header bg-default">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span id="card_title">
                    {!! trans('laravelroles.titles.permissions-card') !!}
                </span>
                ...
                @if(count($items) != 0)
                    @foreach($items as $itemKey => $item)
                        <li id="accordion_permissions_{{ $itemKey }}" class="list-group-item accordion @if($item['roles']->count() > 0 || $item['users']->count() > 0) list-group-item-action accordion-item collapsed @endif" data-toggle="collapse" href="#collapse_permissions_{{ $itemKey }}">
                            ...
                            <div class="d-flex justify-content-between align-items-center" @if($item['roles']->count() > 0 || $item['users']->count() > 0) data-toggle="tooltip" title="{{ trans("laravelroles.tooltips.show-hide") }}" @endif>
                                <span class="badge badge-default permission-name">
                                    {{ $item['permission']->name }}
                                </span>
                                <div>
                                    <span class="badge badge-primary badge-pill">
                                        {!! trans_choice('laravelroles.cards.roles-count', count($item['roles']), ['count' => count($item['roles'])]) !!}
                                    </span>
                                ...
                                <div id="collapse_permissions_{{ $itemKey }}" class="collapse" data-parent="#accordion_permissions_{{ $itemKey }}" >
                                    @if($item['roles']->count() > 0)
                                        <table class="table table-striped table-sm mt-3">
                                            <caption>
                                                {!! trans('laravelroles.cards.permissions-card.permissions-table-roles-caption', ['permission' => $item['permission']->name]) !!}
                                            </caption>
                                            <thead>
                                                <tr>
                                                    <th>{!! trans('laravelroles.cards.permissions-card.role-id') !!}</th>
                                                    <th>{!! trans('laravelroles.cards.permissions-card.role-name') !!}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
